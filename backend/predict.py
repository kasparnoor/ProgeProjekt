...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        video_length: int = Input(
            description="Length of the generated video in seconds.", default=5
        ),
    ) -> Path:
        ...
        # Ensure video length matches the specified video_length
        if video.shape[0] > frames:
            video = video[:frames]

        frame_grids = [
            torchvision.utils.make_grid(frame, nrow=int(args.n_samples))
            for frame in video
        ]  # [3, 1*h, n*w]
